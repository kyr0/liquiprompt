...
const zodiacSign = args[0] || "Taurus"
...
Your natural ${input.zodiac_sign} strengths will be especially valuable this week. Lean into these qualities while remaining aware of potential blind spots.
...
Your natural ${inputValues.zodiac_sign} strengths will be especially valuable this week. Lean into these qualities while remaining aware of potential blind spots.
...
Remember, dear ${inputValues.zodiac_sign}, the stars may guide you, but you create your own destiny through your choices. Embrace the opportunities of this week with openness and intention!
  ...
  // Execute the workflow with mock data in simulation mode
  const result = await runWorkflow(
    ...
    (eventType, data) => {
      if (eventType === "STEP_BEFORE") {
        ...
        console.log(`> Generated text: ${data.result.text ? data.result.text.length : 0} characters`);
    ...
    "simulation" // Use simulation mode instead of making real API calls
  );
  ...
  console.log("\nTotal characters generated: " + result.prev_generation_chars);
}
